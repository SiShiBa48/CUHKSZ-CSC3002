 ...
 * bu to the empty list.  The destructor frees the allocated cells.
 */
 ...
 * -------------------------
 * The put method calls insertKey to search the bu array for a matching
...
void StringMap::rehash(int nBuckets)
{
   ...
   KeyValuePair *lastbu = this->buckets;
   this->nBuckets = nBuckets;
   this->buckets = new KeyValuePair[this->nBuckets];
   clear();
   for (int i = 0; i < len; ++i)
      if (lastbu[i].occupied)
         this->put(lastbu[i].key, lastbu[i].value);
   delete[] lastbu;
}
...
{
   // TODO
   int bu = hashCode(key) % nBuckets;
   if (!buckets[bu].occupied)
   {
      return -1;
   }
   if (buckets[bu].occupied == true && buckets[bu].key == key)
   {
      return bu;
   }
   else
   {
      for (int i = bu + 1; i < nBuckets; i++)
      {

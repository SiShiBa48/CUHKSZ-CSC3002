...
string translateLettersToMorse(string line) {
   ...
   for (int i= 0; i <line.size();i++){
      if ((line[i]>=65)&&(line[i]<=90)){
            string add = map[line.substr(i,1)];
            re = re + add + " ";
      }
   }
   return re;
...
string translateMorseToLetters(string line) {
   // TODO
   map<string,string> imap = createMorseCodeMap();
   map<string,string> map = invertMap(imap);
   string re = "", morse = "";
   for (int i = 0; i<line.size();i++){
      if (line[i] == ' '){
         re = re + map[morse];
...
map<string,string> invertMap(const map<string,string> & m) {
   // TODO
    map<string,string> inmap,map;
    map = createMorseCodeMap();
      for (int i = 65;i<=90;i++){
         char c = i;
         string s = "";
         s = s + c;
